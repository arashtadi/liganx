...
export interface ParsedExtra {
  ...
  /** (F1) MM-GBSA rescoring result. Single-snapshot one-trajectory
   *  ΔG_bind (kcal/mol) computed by OpenMM with Amber14SB + OpenFF
   *  Sage 2.2 + OBC2 implicit solvent. Present only on cells that
   *  have been explicitly rescored via the MM-GBSA endpoint — this is
   *  an opt-in second-pass layer, not auto-run on every dock. The
   *  decomposition (E_complex, E_protein, E_ligand) lets the UI show
   *  the user where the binding energy comes from. See
   *  docs/fep_plus_design.md for context. */
  mmgbsaDg?: number;
  mmgbsaEComplex?: number;
  mmgbsaEProtein?: number;
  mmgbsaELigand?: number;
  mmgbsaMethod?: string;
  mmgbsaSeconds?: number;
  /** Receptor heavy-atom RMSD between minimised and input pose, Å.
   *  Trust signal: ≤0.5 Å healthy, ≤1.0 Å minor drift, >1.0 Å
   *  worrying. Negative = couldn't compute. */
  mmgbsaRmsd?: number;
  /** Ensemble docking telemetry — present only on cells from a job with
   *  ensemble=true. The runner docked this ligand against `total` MD-relaxed
   *  receptor conformers (generated by a short GPU molecular-dynamics run),
   *  `docked` of which produced a usable pose, and kept the best-scoring one.
   *
   *  - `spread` is the kcal/mol gap between the best and worst conformer
   *    scores: how much receptor flexibility actually moved this ligand's
   *    score. ~0 means the pocket was effectively rigid for this ligand;
   *    a large spread means the conformer choice mattered a lot.
   *  - `best` is which conformer won — "input" = the un-relaxed crystal
   *    snapshot (relaxation didn't help this ligand), "confN" = an
   *    MD-relaxed conformer (the pocket had to breathe to fit this ligand).
   *
   *  Backend writes `ensemble=<docked>/<total>|ens_spread=<kcal>|ens_best=<label>`.
   *  The matrix surfaces this as a small "best of N ± spread" subtitle and
   *  PoseDetail expands it into a short explanatory panel. */
  ensemble?: { docked: number; total: number; spread?: number; best?: string };
...
export function parseExtra(extra: string | null | undefined): ParsedExtra {
  ...
  for (const part of extra.split("|")) {
    ...
    switch (k) {
      ...
      case "mmgbsa_dg": {
        const n = parseFloat(v);
        if (Number.isFinite(n)) out.mmgbsaDg = n;
        break;
      }
      case "mmgbsa_e_complex": {
        const n = parseFloat(v);
        if (Number.isFinite(n)) out.mmgbsaEComplex = n;
        break;
      }
      case "mmgbsa_e_protein": {
        const n = parseFloat(v);
        if (Number.isFinite(n)) out.mmgbsaEProtein = n;
        break;
      }
      case "mmgbsa_e_ligand": {
        const n = parseFloat(v);
        if (Number.isFinite(n)) out.mmgbsaELigand = n;
        break;
      }
      case "mmgbsa_method":
        out.mmgbsaMethod = v;
      ...
      case "mmgbsa_seconds": {
        const n = parseFloat(v);
        if (Number.isFinite(n)) out.mmgbsaSeconds = n;
        break;
      }
      case "mmgbsa_rmsd": {
        const n = parseFloat(v);
        if (Number.isFinite(n)) out.mmgbsaRmsd = n;
        break;
      }
      case "vina_terms": {
      ...
      case "ensemble": {
        // Format: "<docked>/<total>" — conformers that produced a pose /
        // conformers docked against. Merge-update so segment order in the
        // extra string doesn't matter.
        const m = v.match(/^(\d+)\/(\d+)$/);
        if (m) {
          out.ensemble = {
            ...(out.ensemble ?? { docked: 0, total: 0 }),
            docked: parseInt(m[1], 10),
      ...
      case "ens_spread": {
        ...
        if (Number.isFinite(n)) {
          out.ensemble = { ...(out.ensemble ?? { docked: 0, total: 0 }), spread: n };
        }
        break;
      }
      case "ens_best": {
        // Winning conformer label: "input" (un-relaxed) or "confN" (relaxed).
        if (v) {
          out.ensemble = { ...(out.ensemble ?? { docked: 0, total: 0 }), best: v };
        }
        break;
      }
